...
[Flags]
enum RuntimeTypeFilterFlags : byte {
    None = 0,
    Concrete = 1 << 0,
    Public = 1 << 1,
    Closed = 1 << 2,
...
abstract record RuntimeTypeFilterExpression {
    public abstract bool IsCacheable { get; }

    public abstract bool Matches(Type type);
}

sealed record DelegateRuntimeTypeFilterExpression(Func<Type, bool> Predicate) : RuntimeTypeFilterExpression {
...
sealed record AndRuntimeTypeFilterExpression(
...
sealed record OrRuntimeTypeFilterExpression(
    RuntimeTypeFilter Left,
    RuntimeTypeFilter Right) : RuntimeTypeFilterExpression {
    public override bool IsCacheable => Left.IsCacheable && Right.IsCacheable;
...
sealed record NotRuntimeTypeFilterExpression(RuntimeTypeFilter Operand) : RuntimeTypeFilterExpression {
